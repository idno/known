<?php
    ...
    namespace known\Entities {
        ...
        use known\Common\ContentType;

        require_once \known\Core\site()->config()->path . '/external/password_compat/lib/password.php';

        class User extends \known\Common\Entity implements \JsonSerializable
        {
            ...
            function __construct()
            {
                ...
                \known\Core\site()->addEventHook('webfinger', function (\known\Core\Event $event) {
                ...
                \known\Core\site()->addEventHook('saved', function(\known\Core\Event $event) {
                    ...
                    if ($user instanceof User) {
                        if ($currentUser = \known\Core\site()->session()->currentUser()) {
                            if ($user->getUUID() == $currentUser->getUUID()) {
                                \known\Core\site()->session()->refreshSessionUser($user);
                            }
                        }
                    }
            ...
            function getIcon()
            {
                $response = \known\Core\site()->triggerEvent('icon', array('object' => $this));
                if (!empty($response) && $response !== true) {
                    return $response;
                }
                if (!empty($this->icon)) {
                    return \known\Core\site()->config()->url . 'file/' . $this->icon;
                }

                return \known\Core\site()->config()->url . 'gfx/users/default.png';
            }
            ...
            function getURL()
            {
                return \known\Core\site()->config()->url . 'profile/' . $this->getHandle();
            }
            ...
            {
                if ($result = \known\Core\site()->db()->getObjects('known\\Entities\\User', array('email' => $email), null, 1)) {
            ...
            {
                if ($result = \known\Core\site()->db()->getObjects('known\\Entities\\User', array('handle' => $handle), null, 1)) {
                    foreach ($result as $row) {
                        return $row;
                    }
                }
		...
		if (preg_match("~".\known\Core\site()->config()->url . 'profile/([A-Za-z0-9]+)?~', $url, $matches))
			return \known\Entities\User::getByHandle ($matches[1]);
		return false;
	    }
            ...
            {
                $apikey       = md5(time() . \known\Core\site()->config()->host . \known\Core\site()->config()->email . rand(0, 999999) . rand(0, 999999) . microtime());
            ...
            function getEditURL()
            {
                return \known\Core\site()->config()->url . 'profile/' . $this->getHandle() . '/edit';
            }
             ...
             *
             * @param \known\Entities\User|string $user
             * @return bool
             */
			...
			\known\Core\site()->events()->dispatch('follow', ['user' => $this, 'following' => $user]);
		    ...
		    \known\Core\site()->events()->dispatch('unfollow', ['user' => $this, 'following' => $user]);
            ...
            function getXAccessGroups($permission)
            {
                ...
                if ($groups = \known\Core\site()->db()->getObjects('known\\Entities\\AccessGroup', array('members.' . $permission => $this->getUUID()), null, PHP_INT_MAX, 0)) {
            ...
            {
                $return = array('PUBLIC', $this->getUUID());
                if ($groups = \known\Core\site()->db()->getRecords(array('uuid' => true),
                    array(
                        'entity_subtype'         => 'known\\Entities\\AccessGroup',
                ...
                if ($temp_types = $this->settings['default_feed_content']) {
                    if (is_array($temp_types)) {
                        foreach ($temp_types as $temp_type) {
                            if ($content_type_class = \known\Common\ContentType::categoryTitleToClass($temp_type)) {
             ...
             * @param \known\Common\Entity|null $object Optionally, an object to pass
            ...
            {
                return \known\Core\site()->triggerEvent('notify', [
                    'user'         => $this,
                    'message'      => $message,
            ...
            /**
             * Save form input
             * @param \known\Common\Page $page
             * @return bool|\known\Common\false|\known\Common\true|\known\Core\false|\known\Core\MongoID|null
             */
            function saveDataFromInput()
            {

                if (!$this->canEdit()) return false;
                $this->profile = \known\Core\site()->currentPage()->getInput('profile');
                if (!empty($_FILES['avatar'])) {
                    if (in_array($_FILES['avatar']['type'], array('image/png', 'image/jpg', 'image/jpeg', 'image/gif'))) {
                        if (getimagesize($_FILES['avatar']['tmp_name'])) {
                            if ($icon = \known\Entities\File::createThumbnailFromFile($_FILES['avatar']['tmp_name'], $_FILES['avatar']['name'], 300)) {
                                $this->icon = (string)$icon;
                            } else if ($icon = \known\Entities\File::createFromFile($_FILES['avatar']['tmp_name'], $_FILES['avatar']['name'])) {
                                $this->icon = (string)$icon;
                            }
                        }
                    }
