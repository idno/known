<?php
    ...
    namespace known\Entities {
        ...
        class AccessGroup extends \known\Common\Entity
        {
            ...
            {
                $this->members = array(
                    'read'  => array(\known\Core\site()->session()->currentUser()->getUUID()),
                    'write' => array(\known\Core\site()->session()->currentUser()->getUUID()),
                    'admin' => array(\known\Core\site()->session()->currentUser()->getUUID())
                );
            ...
            {
                if (empty($user_id)) $user_id = \known\Core\site()->session()->currentUser()->uuid;
            ...
            {
                if ($this->canEdit()) {
                    if ($user = \known\Core\site()->db()->getObject($user_id) && $user instanceof User) {
