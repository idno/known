<?php
    ...
    namespace known\Pages\Entity {
         ...
         * known share screen
         */
        class Share extends \known\Common\Page
        {
            ...
            function getContent()
            {
                ...
                if ($content = \known\Core\Webmention::getPageContent($url)) {
                    if ($mf2 = \known\Core\Webmention::parseContent($content['content'])) {
                        if (!empty($mf2['items'])) {
                ...
                $content_type = \known\Common\ContentType::getRegisteredForIndieWebPostType($share_type);

                if (!empty($content_type)) {
                    if ($page = \known\Core\site()->getPageHandler('/' . $content_type->camelCase($content_type->getEntityClassName()) . '/edit')) {
