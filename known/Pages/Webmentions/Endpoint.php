<?php
    ...
    namespace known\Pages\Webmentions {

        /**
         * Class to serve the webmention endpoint
         */
        class Endpoint extends \known\Common\Page
        {
            ...
            function post()
            {
                ...
                if (!empty($vars['source']) && !empty($vars['target']) && $vars['source'] != $vars['target']) {
                    ...
                    if ($page = \known\Core\site()->getPageHandler($target)) {
                        // First of all, make sure the target page isn't the source page. Let's not webmention ourselves!
                        $webmention_ok = true;
                        if (\known\Common\Entity::isLocalUUID($source)) {
                            if ($source_page = \known\Core\site()->getPageHandler($source)) {
                                if ($source_page == $page) {
                                    $webmention_ok = false;
                                }
                            }
                        ...
                        if ($webmention_ok) {
                            if ($source_content = \known\Core\Webmention::getPageContent($source)) {
                                if (substr_count($source_content['content'], $target) || $source_content['response'] == 410) {
                                    $source_mf2 = \known\Core\Webmention::parseContent($source_content['content'], $source);
