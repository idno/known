<?php
    ...
    namespace known\Pages\Account {
         ...
         */
        class Settings extends \known\Common\Page
        {
            ...
            function postContent()
            {
                ...
                if (!empty($email) && $email != $user->email && filter_var($email, FILTER_VALIDATE_EMAIL)) {
                    if (!\known\Entities\User::getByEmail($email)) {
                        $user->email = $email;
                    } else {
                        \known\Core\site()->session()->addMessage('Someone is already using ' . $email . ' as their email address.', 'alert-error');
                    }
                }
                    ...
                    if (in_array($_FILES['avatar']['type'], array('image/png', 'image/jpg', 'image/jpeg', 'image/gif'))) {
                        if (getimagesize($_FILES['avatar']['tmp_name'])) {
                            if ($icon = \known\Entities\File::createThumbnailFromFile($_FILES['avatar']['tmp_name'], $_FILES['avatar']['name'], 300)) {
                                ...
                                \known\Core\site()->session()->addMessage("Your user picture was updated.");
