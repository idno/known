<?php
    ...
    namespace known\Common {
        ...
        class Entity extends Component implements \JsonSerializable
        {
            ...
            {
                $this->setOwner(\known\Core\site()->session()->currentUser());
            }
            ...
            {
                if ($owner instanceof \known\Entities\User) {
            ...
            function getUUID()
            {
                ...
                if (!empty($this->_id)) {
                    return \known\Core\site()->config()->url . 'view/' . $this->_id;
                }
            ...
            {
                return \known\Core\site()->db()->countObjects(get_called_class(), $search);
            }
            ...
            {
                return \known\Core\site()->db()->countObjects($class, $search);
            }
            ...
            {
                return \known\Core\site()->db()->getObjects(get_called_class(), $search, $fields, $limit, $offset);
            }
            ...
            static function getByID($id)
            {
                try {
                    ...
                    return false; //\known\Core\site()->currentPage()->noContent();
                }
            }
            ...
            {
                $result = \known\Core\site()->db()->getObjects($class, $search, $fields, $limit, $offset);
            ...
            static function isLocalUUID($uuid)
            {
                ...
                // Parse the UUID
                if (($uuid_parse = parse_url($uuid)) && ($url_parse = parse_url(\known\Core\site()->config()->url))) {
            ...
            {
                return \known\Common\ContentType::getContentTypeObjectFromClass($this->getClass());
            }
            ...
            {
                $activityStreamPost = new \known\Entities\ActivityStreamPost();
                $owner              = $this->getOwner();
            ...
            {
                if (!empty($this->owner)) {
                    return \known\Core\db()->getObject($this->owner);
                }
            ...
            function save()
            {
                ...
                if (\known\Core\site()->session()->isLoggedIn() && empty($owner_id)) {
                    $this->setOwner(\known\Core\site()->session()->currentUser());
                }
                ...
                if (\known\Core\site()->triggerEvent('save', array('object' => $this))) { // dispatch('save', $event)->response()) {
                    $result = \known\Core\site()->db()->saveObject($this);
                } else {
                    $result = false;
                }
                if (!empty($result)) {
                    if (empty($this->_id)) {
                        ...
                        \known\Core\site()->db()->saveObject($this);

                        $event = new \known\Core\Event(array('object' => $this));

                        if ($this->getActivityStreamsObjectType()) {
                            \known\Core\site()->events()->dispatch('post/' . $this->getActivityStreamsObjectType(), $event);
                            \known\Core\site()->events()->dispatch('syndicate', $event);
                        }

                        \known\Core\site()->events()->dispatch('saved', $event);
                    }
            ...
            {
                $plugin_slug = \known\Core\site()->triggerEvent('entity/slug', ['object' => $this]);
                ...
                if ($entity = \known\Common\Entity::getBySlug($slug)) {
             ...
             * By default, known entities are objects of type "article".
             *
             * @return string
             */
            ...
            function getRelatedFeedItems($verb = 'post')
            {
                ...
                if ($this instanceof \known\Entities\ActivityStreamPost && $this->verb == $verb) {
                    $results[] = $this;
                }
                ...
                $other_results = \known\Entities\ActivityStreamPost::get($search);
            ...
            function attachFile(\MongoGridFSFile $file_wrapper)
            {
                ...
                $attachments[]     = array('_id' => $file['_id'], 'url' => \known\Core\site()->config()->url . 'file/' . $file['_id'] . '/' . urlencode($file['filename']), 'mime-type' => $file['mime_type'], 'length' => $file['length']);
            ...
            function deleteAttachments()
            {
                if ($attachments = $this->getAttachments()) {
                    foreach ($attachments as $attachment) {
                        if ($file = \known\Entities\File::getByID($attachment['_id'])) {
                            $file->delete();
                        }
                    }
                }
            ...
            function delete()
            {
                ...
                if (\known\Core\site()->triggerEvent('delete', array('object' => $this))) {
                    if ($entries = \known\Entities\ActivityStreamPost::getByObjectUUID($this->getUUID())) {
                        foreach ($entries as $entry) {
                            $entry->delete();
                        }
                    }

                    if ($return = \known\Core\db()->deleteRecord($this->getID())) {
                        $this->deleteData();
            ...
            {
                if (
                    $access instanceof \known\Entities\AccessGroup ||
                    ($access = \known\Core\site()->db()->getObject($access) && $access instanceof \known\Entities\AccessGroup)
                ) {
            ...
            {
                $host     = \known\Core\site()->config()->host;
            ...
            function getShortURL($complete = true, $url_schema = true)
            {
                ...
                if ($complete) {
                    if ($url_schema) {
                        $host = \known\Core\site()->config()->url;
                    } else {
                        $host = \known\Core\site()->config()->host . '/';
                    }
            ...
            {
                $url = str_replace(\known\Core\site()->config()->url . 's/', '', $url);
                if (empty($url)) {
                    return false;
                }
            ...
            function getEditURL()
            {
                return \known\Core\site()->config()->url . $this->getClassSelector() . '/edit/' . $this->getID();
            }
            ...
            {
                return \known\Core\site()->config()->url . $this->getClassSelector() . '/delete/' . $this->getID();
            }
            ...
            function canEdit($user_id = '')
            {
                ...
                if (!\known\Core\site()->session()->isLoggedOn()) return false;

                if (empty($user_id)) {
                    $user_id = \known\Core\site()->session()->currentUserUUID();
                }
            ...
            function canRead($user_id = '')
            {
                ...
                if ($access instanceof \known\Entities\AccessGroup) {
            ...
            function getAccess($idOnly = false)
            {
                ...
                if (!$idOnly && $access != 'PUBLIC') {
                    $access = \known\Core\site()->db()->getObject($access);
                }
            ...
            public function jsonSerialize()
            {
                $object = array(
                    ...
                    'formattedContent'
                                  => \known\Core\site()->template()->autop($this->getDescription()),
            ...
            function getURL()
            {
                ...
                if ($slug = $this->getSlug()) {
                    return \known\Core\site()->config()->url . date('Y', $this->created) . '/' . $slug;
                }
                ...
                return \known\Core\site()->config()->url . $this->getClassSelector() . '/edit';
		...
		$local_url = $this->getURL() . '/annotations/' . md5(time() . $content); // Invent a URL for this annotation
                ...
                \known\Core\site()->triggerEvent('annotation/add/' . $subtype, ['annotation' => $annotation, 'object' => $this]);
