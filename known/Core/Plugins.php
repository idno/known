<?php
    ...
    namespace known\Core {
        ...
        class Plugins extends \known\Common\Component
        {
            ...
            public function init()
            {
                ...
                if (!empty(site()->config()->plugins)) {
                    foreach (site()->config()->plugins as $plugin) {
                        if (is_subclass_of("knownPlugins\\{$plugin}\\Main", 'known\\Common\\Plugin')) {
                            $class                  = "knownPlugins\\{$plugin}\\Main";
             ...
             * @return bool|\known\Common\Plugin
             */
            ...
            {
                $plugins = array();
                if ($folders = scandir(\known\Core\site()->config()->path . '/knownPlugins')) {
                    foreach ($folders as $folder) {
                        if ($folder != '.' && $folder != '..') {
                            if (file_exists(\known\Core\site()->config()->path . '/knownPlugins/' . $folder . '/plugin.ini')) {
                                $plugins[$folder] = parse_ini_file(\known\Core\site()->config()->path . '/knownPlugins/' . $folder . '/plugin.ini', true);
                            }
                        }
                    }
                }
