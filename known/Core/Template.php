<?php
    ...
    namespace known\Core {
        ...
        class Template extends \Bonita\Templates
        {
            ...
            {
                return $this->__(['services' => \known\Core\site()->syndication()->getServices($content_type)])->draw('content/syndication');
            }
            ...
            function drawPagination($count, $items_per_page = null)
            {
                ...
                if ($items_per_page == null) $items_per_page = \known\Core\site()->config()->items_per_page;
            ...
            function parseHashtags($text)
            {
                $r = preg_replace_callback('/(?<!=)(?<!["\'])(\#[A-Za-z0-9]+)/i', function ($matches) {
                    ...
                    return '<a href="' . \known\Core\site()->config()->url . '?q=' . urlencode($matches[1]) . '" class="p-category">' . $url . '</a>';
                }, $text);
            ...
            {
                return \known\Core\site()->config()->url . substr($_SERVER['REQUEST_URI'], 1);
            }
