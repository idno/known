<?php
    ...
    namespace known\Core {
        ...
        class Syndication extends \known\Common\Component
        {
            ...
            {
                \known\Core\site()->events()->addListener('syndicate', function (\known\Core\Event $event) {

                    if (!empty($event->data()['object'])) {
                        $content_type = $event->data()['object']->getActivityStreamsObjectType();
                        if ($services = \known\Core\site()->syndication()->getServices($content_type)) {
                            if ($selected_services = \known\Core\site()->currentPage()->getInput('syndication')) {
                                if (!empty($selected_services) && is_array($selected_services)) {
                                    foreach ($selected_services as $selected_service) {
                                        if (in_array($selected_service, $services)) {
                                            \known\Core\site()->events()->dispatch('post/' . $content_type . '/' . $selected_service, $event);
                                        }
                                    }
                                }
                            }
            ...
            /**
             * Register syndication $service with known.
            ...
            function registerService($service, callable $checker, $content_types = ['article', 'note', 'event', 'rsvp', 'reply'])
            {
                ...
                \known\Core\site()->template()->extendTemplate('content/syndication', 'content/syndication/' . $service);
            }
