<?php
    ...
    namespace known\Core {
        ...
        class Config extends \known\Common\Component
        {
            ...
            function init()
            {
                ...
                $this->url                = (\known\Common\Page::isSSL() ? 'https://' : 'http://') . $_SERVER['SERVER_NAME'] . '/'; // A naive default base URL
            ...
            function save()
            {
                ...
                if (\known\Core\site()->db()->saveRecord('config', $array)) {
                    $this->load();
            ...
            function load()
            {
                if ($config = \known\Core\site()->db()->getAnyRecord('config')) {
                    $config = (array)$config;
                    if (is_array($config)) {
                        $this->config = array_merge($this->config, $config);
                    }
