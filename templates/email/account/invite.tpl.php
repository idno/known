...
<a href="#" style="color: #73b2e3; text-decoration: none;"><?=\Idno\Core\site()->config()->title?></a>.
<br><br>
You can use this space to publish your stories, share things that interest you, and discuss things that matter.
<br><br>
Get started with <strong><?=\Idno\Core\site()->config()->title?></strong> by setting up your account.
<br><br>
<div align="center">
    <a href="<?=\Idno\Core\site()->config()->getURL()?>account/register/?email=<?=urlencode($vars['email'])?>&code=<?=urlencode($vars['code'])?>" style="background-color:#73B2E3;border:1px solid #73B2E3;border-radius:4px;color:#ffffff;display:inline-block;font-family:sans-serif;font-size:17px;font-weight:normal;line-height:40px;text-align:center;text-decoration:none;width:200px;-webkit-text-size-adjust:none;mso-hide:all;">Sign up now</a></div>
    <br>
    If you have any questions at all, please don't hesitate to contact us by sending an email to
    <a href="mailto:<?=\Idno\Core\site()->config()->from_email?>" style="color: #73b2e3; text-decoration: none;"><?=\Idno\Core\site()->config()->from_email?></a>.
    <br><br>
</div>