<div id="form-main">
    <div id="form-div">
        <h2 class="register"><?= \Idno\Core\Idno::site()->language()->_('Hello there!'); ?></h2>

        <p><?= \Idno\Core\Idno::site()->language()->_('Create a new account to get started.'); ?></p>
        ...
        <form action="<?= \Idno\Core\Idno::site()->config()->getDisplayURL() ?>begin/register" method="post"
            ...
            <p class="username">
                <label class="control-label" for="inputUsername"><?= \Idno\Core\Idno::site()->language()->_('Choose a username'); ?><br/></label>
                <input name="handle" type="text" class="feedback-input" placeholder="username" id="username"
                       autocapitalize="off" required/>
            </p>

            <p class="email">
                <label class="control-label" for="inputUsername"><?= \Idno\Core\Idno::site()->language()->_('Your email address'); ?><br/></label>
                <input name="email" type="email" class="feedback-input" id="email" placeholder="you@email.com"
                       autocapitalize="off" required/>
            </p>

            <p class="password">
                <label class="control-label" for="inputUsername"><?= \Idno\Core\Idno::site()->language()->_('Create a password'); ?>
                    <small><?= \Idno\Core\Idno::site()->language()->_('(at least 7 characters please)'); ?></small>
                    <br/></label>
            ...
            <p class="password">
                <label class="control-label" for="inputUsername"><?= \Idno\Core\Idno::site()->language()->_('Your password again'); ?><br/></label>
