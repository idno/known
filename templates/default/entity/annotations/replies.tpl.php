        ...
        foreach ($vars['annotations'] as $locallink => $annotation) {
            ...
            <div class="idno-annotation row u-comment h-cite">
                <div class="idno-annotation-image col-md-1 hidden-sm">
                    <p>
                        <a href="<?= strip_tags($annotation['owner_url']) ?>" class="icon-container"><img
                                    src="<?= \Idno\Core\Idno::site()->config()->sanitizeAttachmentURL(strip_tags($annotation['owner_image'])) ?>"/></a>
                    </p>
                </div>
                <div class="idno-annotation-content col-md-9">
                    ...
                    <span class="p-author h-card">
                        <a class="u-photo" href="<?= strip_tags($annotation['owner_image']) ?>"></a>
                        <a class="p-name u-url"
                           href="<?= htmlspecialchars(strip_tags($annotation['owner_url'])) ?>"><?= htmlentities($annotation['owner_name'], ENT_QUOTES, 'UTF-8') ?></a></span>,
