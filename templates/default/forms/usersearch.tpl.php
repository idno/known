...
?>
<form id="<?= $vars['id']; ?>" class="users-search <?php
if (!empty($vars['class']))
    echo $vars['class'];
?>" method="GET" action="<?= $vars['source-url']; ?>">
...
<script>
	...
	//if (query.val().length > 0) {
		...
		success: function(data) {
		    ...
		    form.find('.results').html(data.rendered);
                    
                    // Handle pagination
                    form.find('.pager li').addClass('pagination-disabled');
                    if (offset > 0) form.find('.pager li.newer').removeClass('pagination-disabled');
    ...
    executeSearch(form); // Load initial
